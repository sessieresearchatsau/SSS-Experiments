...
\tRulePlacement \[Rule] Bottom | Top | Left | Right | None (or False) specifies where to place the rulelist icon relative to the SSS visual display (if shown).  
...
(* Roll the ICat forward or backward *)
...
RollIC[{ic_IndexedConcatenate}] := {RollIC[ic]};
...
RollBackIC[{ic_IndexedConcatenate}] := {RollBackIC[ic]}

(* place to put the ReduceToIC2 code *)
